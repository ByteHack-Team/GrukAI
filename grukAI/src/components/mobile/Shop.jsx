...
function Shop() {
  ...
  // Mock shop items - Eco-friendly & Aesthetic
  const shopItems = [
    // 🌍 Everyday Aesthetic & Useful
  ...
  const handleRedeem = (item) => {
    if (userPoints >= item.points) {
      ...
      alert(`Not enough points! You need ${item.points - userPoints} more points.`)
    }
  }

  const handleRedeemConfirm = (item, userInfo) => {
    ...
    alert(`Successfully redeemed ${item.name}! ${
      item.category === 'Digital & Lifestyle' 
        ? 'You will receive it via email within 24 hours.' 
          ...
          {filteredItems.map(item => (
            <div key={item.id} className="bg-white/80 backdrop-blur-sm rounded-2xl p-4 border border-emerald-200/30 shadow-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  {userPoints >= item.points ? 'Redeem' : 'Need More'}
                </button>
              </div>
            </div>
          ))}
